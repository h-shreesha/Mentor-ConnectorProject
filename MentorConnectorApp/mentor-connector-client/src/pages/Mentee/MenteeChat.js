...
function MenteeChat() {
    ...
    const getMyRequets = () => {
        Axios.post("http://127.0.0.1:3001/getmentorlist", {
            ...
            setChatId(response.data[0].mentor_info._id);
            setChatName(response.data[0].mentor_info.name);
            setChatEmail(response.data[0].mentor_info.email);
            setChatPhone(response.data[0].mentor_info.phone);
        });
    }
                ...
                <div className="row mb-5">
                    {
                        requestList.map((data, key) => {
                            return (
                                <div className="col-12 mb-3" onClick={() => getChat(data.mentor_info._id, data.mentor_info.name, data.mentor_info.email, data.mentor_info.phone)}>
                                    <div className="card">
                                        <div className="card-body">
                                            <h5 className="mb-1 p-0"><b>{data.mentor_info.name}</b></h5>
                                            <p className="m-0 p-0">{data.mentor_info.email}</p>
                                        </div>
                                    </div>
                                </div>
                            );
